...
def min_pops(dictionary, min_val):
    ...
    for pairval, numpairs in dictionary.items():
        if len(numpairs) > min_val:
            ...
            excluded += len(numpairs)
    return popular_pairs, len(popular_pairs), excluded
...
#pairs, _, _ = min_pops(atkmvspd_pairs, 10)
#print (pairs)

for cyc in range(pair_sizes):
    _, popularity[cyc], _ = min_pops(atkmvspd_pairs, cyc)
...
min_encount = 5
pop_pair, number, excluded = min_pops(atkmvspd_pairs, min_encount)
print ("The number of atk movespeed pairs with at least ", min_encount, " enemies are ", number)
#print (pop_pair)
for cyc1 in range(number):
    print ("The pair of interest is ", pop_pair[cyc1], ' ', atkmvspd_pairs[pop_pair[cyc1]])
    listofkeys = random_keys(enemies, atkmvspd_pairs[pop_pair[cyc1]], 3)
    display_images(listofkeys, sprites_folder)
    #print (cyc)

print ("The number of enemies that don't fall under these classifications are ", excluded)
